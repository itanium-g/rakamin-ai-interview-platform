...
async function measurePing(): Promise<number> {
    if (SPEED_TEST_PING_URL) {
        try {
            ...
            await fetchWithTimeout(SPEED_TEST_PING_URL, { cache: "no-cache" }, 4000);
    ...
    for (const url of testUrls) {
        try {
            const start = performance.now();
            await fetchWithTimeout(url, { mode: "no-cors", cache: "no-cache" }, 3000);
            return performance.now() - start;
        } catch {
            continue;
        }
...
async function measureDownloadSpeed(): Promise<number> {
    ...
    for (const testFile of testFiles) {
        try {
            const start = performance.now();
            const response = await fetchWithTimeout(testFile.url, { cache: "no-cache" }, 5000);
            if (response.ok) {
                await response.blob();
    ...
    try {
        const start = performance.now();
        await fetchWithTimeout("https://www.google.com/favicon.ico", { mode: "no-cors", cache: "no-cache" }, 3000);
...
async function measureUploadSpeed(): Promise<number> {
    ...
    for (const endpoint of endpoints) {
        try {
            ...
            const start = performance.now();
            await fetchWithTimeout(endpoint, { method: "POST", body: formData }, 5000);
            const seconds = (performance.now() - start) / 1000;
